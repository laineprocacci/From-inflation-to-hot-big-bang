...
(*metric is the same, but now all sums run from 1 to 3!*)
...
(*cristoffel symbols*)
 ...
 gamma[rho_,mu_,nu_]:=1/2 Sum[gup[rho,sigma](D[gdown[sigma,mu],x[nu]]+
  D[gdown[sigma,nu],x[mu]]-D[gdown[mu,nu],x[sigma]]),{sigma,1,3}]

(*ricci tensor*)

 riccidown[mu_,nu_]:=Sum[D[gamma[alpha,mu,nu],x[alpha]],{alpha,1,3}]-
  Sum[D[gamma[alpha,mu,alpha],x[nu]],{alpha,1,3}]+
  Sum[gamma[beta,mu,nu]gamma[alpha,beta,alpha],{alpha,1,3},{beta,1,3}]-
  Sum[gamma[beta,mu,alpha]gamma[alpha,beta,nu],{alpha,1,3},{beta,1,3}]

(*raise first index in ricci tensor*)

 ricciupdown[mu_,nu_]:=Sum[gup[mu,alpha]riccidown[alpha,nu],{alpha,1,3}]

(*spatial ricci scalar*)

 ricciscalar:=Sum[ricciupdown[alpha,alpha],{alpha,1,3}]

(*adjust expression until the difference subtracts to zero*)

  calH=a'[x[0]]/a[x[0]];
  calHp=a''[x[0]]/a[x[0]] - (a'[x[0]]/a[x[0]])^2;
  testricciscalar=-2 eps/a[x[0]]^2(  
      + 0 calH Derivative[1,0,0,0][h0][x[0],x[1],x[2],x[3]] 
      + 0 (calHp+ 2 calH^2) Derivative[0,0,0,0][h0][x[0],x[1],x[2],x[3]] 
      + 0 Derivative[0,2,0,0][h0][x[0],x[1],x[2],x[3]] 
      + 0 Derivative[0,0,2,0][h0][x[0],x[1],x[2],x[3]] 
      + 0 Derivative[0,0,0,2][h0][x[0],x[1],x[2],x[3]] 
      ...
      + 0 Derivative[2,0,0,0][hD][x[0],x[1],x[2],x[3]]  
      + 0 calH Derivative[1,0,0,0][hD][x[0],x[1],x[2],x[3]]  
       - 0 Derivative[1,1,0,0][h1][x[0],x[1],x[2],x[3]] 
       - 0 Derivative[1,0,1,0][h2][x[0],x[1],x[2],x[3]] 
       - 0 Derivative[1,0,0,1][h3][x[0],x[1],x[2],x[3]] 
       - 0 calH Derivative[0,1,0,0][h1][x[0],x[1],x[2],x[3]] 
       - 0 calH Derivative[0,0,1,0][h2][x[0],x[1],x[2],x[3]] 
       - 0 calH Derivative[0,0,0,1][h3][x[0],x[1],x[2],x[3]] 
       - 0 Derivative[2,0,0,0][theta11][x[0],x[1],x[2],x[3]] 
       - 0 Derivative[2,0,0,0][theta22][x[0],x[1],x[2],x[3]] 
       - 0 Derivative[2,0,0,0][theta33][x[0],x[1],x[2],x[3]] 
       - 0 calH Derivative[1,0,0,0][theta11][x[0],x[1],x[2],x[3]] 
       - 0 calH Derivative[1,0,0,0][theta22][x[0],x[1],x[2],x[3]] 
       - 0 calH Derivative[1,0,0,0][theta33][x[0],x[1],x[2],x[3]] 
      - Derivative[0,2,0,0][theta11][x[0],x[1],x[2],x[3]]
      - Derivative[0,0,2,0][theta22][x[0],x[1],x[2],x[3]]
      - Derivative[0,0,0,2][theta33][x[0],x[1],x[2],x[3]]
